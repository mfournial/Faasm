...
int main(int argc, char *argv[]) {
    ...
    std::string outfile = std::string("/usr/local/code/faasm/wasm/omp/")  + funcName + "/bench_wasm.csv";
...
void wasmRun(const std::string &user, const std::string &funcName, int wasmIterations, const std::string &outfile) {
    ...
    for (int run = 1; run <= wasmIterations; run++) {
        wasmRunIteration(profOut, call, run, "2", 20, cachedModule);
...
//    call.set_cmdline(args.c_str());
    ...
    module.execute(call);
    const long wasmIterationTime = util::getTimeDiffMillis(iterationTp);

    profOut << num_threads << ",wasm-pool," << wasmIterationTime << std::endl;
}
